...
class LegitimacyAnalyzer:
    ...
    def _analyze_ssl(self, ssl_info):
        ...
        # Check expiration (higher score for certificates with longer validity)
    ...
    def _analyze_content(self, content):
        ...
        if '<form' in content.lower():
            if 'method="post"' in content.lower() and 'https://' in content.lower():
                ...
                content_score -= 0.2  # Penalize insecure forms

        # Check for suspicious obfuscation
        ...
        # Check for excessive popups/redirects
        if any(x in content.lower() for x in ['window.location', 'document.location', 'window.open']):
            if content.lower().count('window.open') > 3:
                content_score -= 0.1

        # Base legitimacy score
        content_score += 0.7  # Start with a base score that can be reduced
