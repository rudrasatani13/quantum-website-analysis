...
class AIDetector:
  ...
  def _detect_anomalies(self, content: str, features: Dict[str, Any]) -> List[Dict[str, Any]]:
      ...
      try:
          ...
          # Check for behavioral anomalies - Adjusted thresholds
  ...
  def _match_patterns(self, content: str) -> List[Dict[str, Any]]:
      ...
      try:
          ...
          advanced_patterns = {
              ...
              'file_inclusion': {'pattern': r'(?:include|require|import)\s*\(\s*["\']?(?:\.\./|/etc/|/proc/|/var/)', 'base_confidence': 0.7},
              'crypto_mining': {'pattern': r'(?:stratum|mining|hashrate|cryptocurrency|bitcoin|ethereum|monero)', 'base_confidence': 0.4},
              'data_exfiltration_advanced': {'pattern': r'(?:ftp|sftp|scp|rsync|wget|curl)\s+.*(?:upload|send|transfer|copy)', 'base_confidence': 0.5}
          }
  ...
  def _get_pattern_severity(self, pattern_name: str) -> str:
      ...
      high_severity_patterns = [
          'obfuscated_javascript', # Keep this as high if it triggers, but it should be harder to trigger now
      ...
      medium_severity_patterns = [
          ...
          'crypto_mining' # Added crypto_mining to medium
      ]
